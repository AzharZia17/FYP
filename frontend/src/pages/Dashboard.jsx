...
const Dashboard = () => {
    ...
    const handleDeptClick = async (dept) => {
        ...
        try {
            const stats = await dashboardAPI.getSemesterStats(dept);
            setSemesterStats(stats);
        } catch (err) {
            console.error("Semester stats catch:", err);
        } finally {
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-8 pb-10">
                ...
                <div className="flex justify-between items-end">
                    <div>
                        ...
                        <p className="text-slate-400">Holistic view of participation across departments and semesters.</p>
                    </div>
                    <button
                        onClick={fetchStats}
                        className="p-3 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-2xl transition-all border border-slate-700 active:scale-90"
                    >
                ...
                <div className="space-y-6">
                    <h2 className="text-xl font-bold text-white pl-1 flex items-center gap-2">
                        <span className="w-2 h-8 bg-cyan-500 rounded-full block" />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        {deptStats.map((dept, i) => (
                            <motion.button
                                ...
                                className={`p-6 rounded-3xl text-left border transition-all ${selectedDept === dept.department
                                    ? 'bg-cyan-500/10 border-cyan-500/50 shadow-[0_0_20px_rgba(6,182,212,0.1)]'
                                    : 'glass-effect border-slate-800 hover:border-slate-700'
                                    }`}
                            >
                                <h4 className={`font-bold mb-4 ${selectedDept === dept.department ? 'text-cyan-400' : 'text-slate-300'}`}>
                                    {dept.department}
                                </h4>
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">Present</p>
                                        <p className="text-lg font-bold text-emerald-400">{dept.present}</p>
                                    </div>
                                </div>
                            </motion.button>
                        ))}
                    ...
                    {selectedDept && (
                        <motion.div
                            ...
                            className="glass-effect rounded-[2.5rem] border border-cyan-500/20 overflow-hidden"
                        >
                            <div className="p-8 border-b border-white/5 flex justify-between items-center bg-cyan-500/5">
                                <h3 className="font-bold text-white">Semester Breakdown for <span className="text-cyan-400">{selectedDept}</span></h3>
                                <button onClick={() => setSelectedDept(null)} className="text-xs text-slate-500 hover:text-white uppercase font-bold">Close Panel</button>
                            </div>
                            <div className="p-8">
                                {isDrillDownLoading ? (
                                    <div className="flex justify-center py-10"><RefreshCw className="animate-spin text-cyan-500" /></div>
                                ) : (
                                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
                                        {semesterStats.map((s) => (
                                            <div key={s.semester} className="p-4 bg-slate-900/50 rounded-2xl border border-slate-800 text-center">
                                                <p className="text-[10px] text-slate-500 font-bold uppercase mb-1">Sem {s.semester}</p>
                                                <div className="text-lg font-bold text-white">{s.present}/{s.total}</div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="glass-effect p-8 rounded-[2.5rem] border border-slate-800/60">
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <h2 className="text-xl font-bold text-white">Status Mix</h2>
                        </div>
                ...
                <div className="glass-effect rounded-[2.5rem] border border-slate-800/60 overflow-hidden">
                    <div className="p-8 border-b border-slate-800/40 flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-bold text-white">Live System Activity</h2>
                    </div>
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-slate-900/50 text-[10px] font-bold uppercase tracking-widest text-slate-500">
                                    ...
                                    <th className="px-8 py-4 text-center">Triggered At</th>
                                ...
                                {recent_activity.map((log, i) => (
                                    <tr key={i} className="group hover:bg-white/5 transition-all">
                                        ...
                                        <td className="px-8 py-5 text-right">
                                            <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${log.status === 'present' ? 'bg-emerald-500/10 text-emerald-400' :
